...
class SettingsPanel {
    ...
    async handleCancelSubscription() {
        // PATCHED: No external API call
        vscode.window.showInformationMessage('Subscription management disabled in free/local mode.');
    }

    async handleCheckPro() {
        // PATCHED: Always succeeds - no external verification
        await this.context.globalState.update('auto-accept-isPro', true);
        vscode.window.showInformationMessage('Auto Accept: Pro features active!');
        this.update();
    }

    isPro() {
        return true; // PATCHED: Always pro
    }
    ...
    async checkProStatus(userId) {
        // PATCHED: Always return true - no external API call
        return true;
    }

    startPolling(userId) {
        // PATCHED: No API polling needed - always pro
    }
}
